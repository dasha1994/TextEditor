...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
	    ...
	    // getNumSyllables method in BasicDocument (module 1) and
		   List syllables = getTokens("[?ayuioeAEYUIO]+",word);
           int count = syllables.size();

		   if(syllables.size()>=2 && word.endsWith("e"))
		   {
			   if(!(word.charAt(word.length()-2)=='e' || word.charAt(word.length()-2)=='a'
					   || word.charAt(word.length()-2)=='y' || word.charAt(word.length()-2)=='u'
